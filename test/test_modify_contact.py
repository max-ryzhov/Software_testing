...
def test_update_contact(app):
    if app.contact.count() == 0:
        app.contact.create(ContactConstruct(firstname="Default", lastname="User", address="for_modification"))
    mod_contact = ContactConstruct(firstname="mod_name", lastname="mod_lastname", address="mod_address", mobile="+0000",
    ...
    assert sorted(old_contacts, key=ContactConstruct.id_or_max) == sorted(mod_contacts, key=ContactConstruct.id_or_max)
