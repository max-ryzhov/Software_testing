...
def test_modify_group_name(app):
    mod_group = GroupConstruct(group_name="Modified name")    # назвал модифицированную группу mod_group
    if app.group.count() == 0:    # если групп нет, то создать
        app.group.create(GroupConstruct(group_name="For modify"))
...
def test_modify_group_name2(app):
    if app.group.count() == 0:    # если групп нет, то создать
        app.group.create(GroupConstruct(group_name="For modify2"))
    old_groups = app.group.get_group_list()
    # Сразу передаю id 1й групп в конструктор mod_group
    mod_group = GroupConstruct(group_name="Second modification", group_id=old_groups[0].group_id)
